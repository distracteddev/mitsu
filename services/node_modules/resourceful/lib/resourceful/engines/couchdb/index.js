...
Couchdb.prototype.view = function (path, opts, callback) {
  return this.request.call(this, 'view', path, opts, function (e, res) {
    ...
    callback(null, res.rows.map(function (r) {
      // Check to see if the view returns an object that represents a Resource.
      // With `include_docs=true` option set, the 'doc' attribute is used instead
      // of 'value', so we must check both
      if ((r.doc && typeof r.doc.resource === "undefined") || (r.value && typeof r.value.resource === "undefined")) {
        // Since the view doesn't return a resource, we assume the user requires
        // both the key and value of the returned rows.
        var row = {key: r.key, value: r.value};
        return row;                  
      }
      else {        
