  ...
  "A Resource definition with filters": {
    topic: function () {
      Article = resourceful.define('Article', function () {
        ...
        this.filter('tagFilter', {group: true}, {
          // Should return an array of key/value pairs where the keys are a list
          // of distinct tags, and the value is that tag's frequency of occurence.
          map: function (post) {
    ...
    "can be used to query the database:": {
      ...
      "<tagFilter>": {
        topic: function (Article) {
          Article.tagFilter(this.callback);
        },
        "Should return an array of Article tags as values, and their frequency as keys": function (e, res) {
          assert.isArray(res);
          assert.equal(res.length,6);
          assert.equal(res[0].key, "classic");
          assert.equal(res[0].value, 1);
          assert.equal(res[1].key, "force");
          assert.equal(res[1].value, 1);
          assert.equal(res[2].key, "hacking");
          assert.equal(res[2].value, 1);
          assert.equal(res[3].key, "vi");
          assert.equal(res[3].value, 2);
          assert.equal(res[4].key, "writing");
          assert.equal(res[4].value, 1);
          assert.equal(res[5].key, "zen");
